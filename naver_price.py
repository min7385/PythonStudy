...
sql_merge = """
                ...
                WHEN NOT MATCHED THEN
                    ...
                    VALUES (:code, :discussionId, :readCount, :goodCount, :badCount,
                            :commentCount, :title, :contents, to_date(:createDt, 'YYYY-MM-DD HH24:MI:SS'))
"""
...
def get_price(code):
    ...
    if res.status_code == 200:
        ...
        log.info(f"{res.status_code} 응답으로, 현재 가격 아님")
def get_bbs(code):
...
def current_price():
    ...
    try:
        ...
        conn.commit()
        log.info("current price 정상 저장 완료")
    finally:
        conn.close()
        log.info("current pricec 완료")
